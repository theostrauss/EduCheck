...
public class EduCheck {
    ...
    let names = ["aamu.edu" : "Alabama A&M University",
    ...
    /**
     ...
     Check if an email address input for a university matches the university's domain
     
     - author:
     Theodore Strauss
     
     - returns:
     A boolean that lets one know if the domain matches the university's
     
     - parameters:
         - string1: The email address that one wants to check. It cannot be empty. It is recommended that one creates a variable holding the string before inserting it into the function parameter.
     
     - important:
     This framework contains the names and domains of 900+ universities in the USA. Although universities are not founded frequently, let it be known that this list was created in 2017.
    ...
    public func eduCheck(input: String) -> Bool {
        
        var match = false //set inital return value as false
        
        for name in names.keys { //loops through every value of keys
            if input.hasSuffix("@\(name)") { //if the user's input has @key on the end of it,
                print("EduCheck 🎓✔️: The university is \(names[name]!)") //prints the value
                match = true //changes match to true, making the function return that the input matches the key
            } else {
                match = false //leaves the return value as false, alerting the developer that the email domain does not match
            }
        }
        return match
    }
